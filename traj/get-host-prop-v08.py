    ...
    for _ in range(2):
        ...
        for ring in atom_rings:
            ...
            for atom_idx in ring:
                ...
                if len(neighbors_in_main_cycle) > 0:
...
def get_cycle_props(mol, host_name):
    ...
    for ref_atom in main_cycle_atoms:
        ...
        if len(cycle_neighbors_13.union(main_cycle_atoms)) == n_atoms_total:
            break
    ...
    electroneg_nieghs = np.array([neigh_idx if cycle_neighbors_symbols[n] in target_symbols else None
                                  for n, neigh_idx in enumerate(cycle_neighbors_13)])
    ...
    # Count atom types in ring (INCLUSIVE ring set, as before)
...
# ================== MINIMAL-CHANGE ADDITION (from your 2nd code) ==================
def detect_large_cycles(mol):
    """
    Same algorithm as in your 2nd script:
    - collect atoms in any ring with len >= 8
    - count unique ring atoms by element
    - also compute an "electronegative neighbor" proxy (kept for completeness)
    """
    ri = mol.GetRingInfo()
...
# ================== MINIMAL-CHANGE FIX YOU REQUESTED ==================
# Override ONLY num_cycle_carbons using the 2nd-code algorithm
# (everything else stays from get_cycle_props)
_, num_cycle_c_fix, _, _, _, _ = detect_large_cycles(mol)
num_cycle_c = int(num_cycle_c_fix)  # ensure it's a plain int for the CSV
# ==================================================
...
host_props = pd.DataFrame({
    ...
    'num_cycle_carbons': num_cycle_c,   # <-- now uses 2nd-code algorithm
}, index=[0])
