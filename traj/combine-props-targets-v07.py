...
for host_name in dG.index:
    try:
        ...
        all_prop = pd.read_csv(
            f'all-host-props-nopore/host_{host_name}_all_props-reviewed.csv',
            delimiter=',',
            header=0
        )
...
all_data.to_csv('all-host-props-nopore/all_host_props_training-reviewed.csv', index=False)

print("Wrote: all-host-props-nopore/all_host_props_training-reviewed.csv")
