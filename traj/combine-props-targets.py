...
for h, host_name in enumerate(host_names):
    # Read property data
    all_prop = pd.read_csv(f'host-props/host_{host_name}_all_props.csv', delimiter=',', header=0, index_col=0)
...
# write to csv
all_data.to_csv(f'host-props/all_host_props.csv', index=True)
